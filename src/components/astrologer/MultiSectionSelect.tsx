    ...
    <div ref={rootRef}>
      ...
      <div className="relative mt-1.5">
        <button
          ...
          onClick={() => setOpen((v) => !v)}
          className={cn(
            "flex w-full items-center justify-between gap-2 rounded-xl border bg-surface px-3 py-2.5 text-left text-sm transition",
            open
              ? "border-[#F06A00]/55 ring-[3px] ring-[#F06A00]/12"
              : "border-white/12 hover:border-[#F06A00]/35"
          )}
        >
          <span
            className={cn(
              "min-w-0 truncate",
              values.length ? "font-medium text-ink" : "text-ink-muted"
            )}
          >
            {values.length
              ? values
        ...
        {open ? (
          <div
            ...
            className="absolute left-0 right-0 top-[calc(100%+0.5rem)] z-40 max-h-60 overflow-y-auto overscroll-contain rounded-xl border border-white/12 bg-surface shadow-[0_16px_40px_-18px_rgba(42,33,24,0.45)]"
          >
            <div className="p-2 pb-3">
              {sections.map((section, idx) => (
                <div key={section.id} className={cn(idx > 0 && "mt-2")}>
                  <p className="sticky top-0 z-10 -mx-2 border-b border-white/10 bg-surface px-4 py-2 text-[10px] font-bold uppercase tracking-[0.12em] text-ink-muted">
                        ...
                        <li key={opt.id}>
                          <button
                            ...
                            className={cn(
                              ...
                              on
                                ? "bg-cosmic-purple/15 font-semibold text-cosmic-gold"
      ...
      {values.length > 0 ? (
        <div className="mt-1.5 flex flex-wrap gap-1">
          {values.map((id) => (
            <span
              key={id}
              className="inline-flex items-center gap-1 rounded-md bg-cosmic-purple/15 px-1.5 py-0.5 text-[11px] font-semibold text-cosmic-gold"
            >
              {labels.get(id)}
              <button
                type="button"
