...
export function LoShuResult({
  ...
  return (
    <div className="space-y-8">
      {/* Hero summary */}
      <div className="rounded-2xl border border-saffron/20 bg-gradient-to-br from-[#fff7f0] via-white to-[#ffe8d4]/50 p-5 sm:p-6">
        <div className="flex flex-wrap items-end justify-between gap-3">
          ...
          <div className="flex gap-2 text-center">
            <div className="rounded-xl bg-white/80 border border-black/5 px-3 py-2 min-w-[4.5rem]">
          ...
          <div className="space-y-3">
            {present.map((item) => (
              <div
                ...
                className="flex gap-3 rounded-2xl border border-black/8 bg-white p-4"
              >
        ...
        <div>
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            {missing.map((item) => (
              <div
                key={String(item.number)}
                className="rounded-2xl border border-dashed border-black/15 bg-sand/20 p-4"
              >
                <p className="font-semibold text-ink text-[14px]">
                  <span className="inline-flex h-7 w-7 items-center justify-center rounded-lg bg-white border border-black/8 text-ink-muted mr-2 text-sm font-bold tabular-nums">
                    {String(item.number)}
                  </span>
                  {t(locale, item.title as Loc)}
                </p>
